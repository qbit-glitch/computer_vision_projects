...
def load_images_and_anns(im_sets, label2idx, ann_fname, split):
    """
    Method to get the xml files and for each file get all the objects and their ground truth detection imformation for the dataset
    """
    im_infos = []
    ims = []
    ...
    for im_set in im_sets:
        im_names = []
        # Fetch all image names in txt file for this image set
        for line in open(os.path.join(im_set, 'ImageSets', 'Main', '{}.txt'.format(ann_fname))):
        ...
        for im_name in im_names:
            ...
            for ob in ann_info.findall('object'):
                det = {}
...
class VOCDataset(Dataset):
    def __init__(self, split, im_sets, im_size=448, S=7, B=2, C=20):
        ...
        # Train and test transformations for AUGMENTATIONS
        self.transforms = {
            'train': albu.Compose([
        ...
        classes = [
            ...
            'chair', 'diningtable', 'pottedplant', 'sofa', 'tymonitor'
        ]
    ...
    def __getitem__(self, index):
        ...
        bboxes = torch.as_tensor(transformed_info['boxes'])
        ...
        im_tensor_channel_2 = (torch.unsqueeze(im_tensor[0], 0) - 0.406) / 0.225
        ...
        # Height and width of grid cells is H // 5
        ...
        if len(bboxes) > 0:
            ...
            box_center_y = bboxes_tensor[:, 1] + 0.5 * box_widths
        ...
        # For training, we use yolo_tagets[xoffset, yoffset, sqrt(w), sqrt(h)]
        # For evaluation we use bboxes_tensor(x1, y1, x2, y2)
        # Below we normalize bboxes tensor to be between 0-1
        # as that's what evaluation script expects so (x1/w, y1/h, x2/w, y2/h)
